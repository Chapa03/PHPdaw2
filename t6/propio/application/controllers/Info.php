...
class Info extends CI_Controller {
    public function index() {
        ...
        //en la vista
        session_start();
        $data['msg'] = isset($_SESSION['_msg']['text']) ? $_SESSION['_msg']['text'] : 'Vuelve al menú principal';
        $data['severity'] = isset($_SESSION['_msg']['severity']) ? $_SESSION['_msg']['severity'] : 'info';
        //Si por sesión se indica a donde se quiere volver, uri valdrá esa ruta
        //sino, valdrá la ruta base de la aplicación
        $data['uri'] = base_url() . (isset($_SESSION['_msg']['uri']) ? $_SESSION['_msg']['uri'] : '');
